...
void led_name(int count){
	...
	switch(a){
		case 1:{
			...
			usleep(100000);
			break;
		}//player1 이름 첫번째 단어 입력되면 왼쪽부터 2개의 led 점멸  
		case 2:{
			data=0xF0;
			write(dev, &data, sizeof(unsigned char));
			usleep(100000);		
			...
			break;
		}//player1 이름 두번째 단어 입력되면 왼쪽부터 4개의 led 점멸 
		case 3:{
			data=0xc0;
			write(dev, &data, sizeof(unsigned char));
			usleep(100000);
			break;
		}//player2 이름 첫번째 단어 입력되면 왼쪽부터 6개의 led 점멸 
		case 4:{
			data=0x00;
			write(dev, &data, sizeof(unsigned char));
			usleep(100000);
			break;
		}//player2 이름 두번째 단어 입력되면 왼쪽부터 8개의 led 점멸 
	}
	close(dev);
}// intro_game함수에서 이름 입력시 몇번째 단어가 입력榮쩝 확인할수 있게 하는 함수  
...
void append_name(char* dst, char c) {
	...
	*p = c;
	*(p + 1) = '\0';
}//player1_name , player2_name 배열에 문자를 저장하는 함수  
...
int intro_game() {
	...
	int i = 0;
	int count = 0;
	char alphP[26] = "ABCDEFGHIJKLMNOPQRSTUVWXYZ"; //알파벳 A~Z 배열  
		...
		if ((dotend1.tv_usec - dotst1.tv_usec > 200000) || (dotend1.tv_sec > dotst1.tv_sec && (dotend1.tv_usec + 1000000 - dotst1.tv_usec > 200000)))
		{
			...
			while (1) {
				...
				case KEY_NUM5: {
					...
					led_name(count);
					if (count < 3) {
						append_name(player1_name, alphP[i]);					
					}
					else {
						append_name(player2_name, alphP[i]);
					}

					break;
				}//알파벳 선택  

				case KEY_NUM6://오른쪽으로 이동  
					i = i + 1;
					break;
				}
